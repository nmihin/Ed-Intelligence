...
new Vue({
    ...
    methods: {
        ...
        loadGroup: function(group) {
            ...
            _this.$http.get(group_path + '/description.html' + '?cb=' + new Date()).then(function (response) {
                group.description = marked(response.data);
                _this.areGroupsLoaded();
            }, function () {
                _this.logError('Description file for <strong>' + group.name + '</strong> group failed to load from <code>' + group_path + '/description.html</code>');
            });
        },
        ...
        loadComponent: function(component) {
            ...
            _this.$http.get(component_path + '/markup.html' + '?cb=' + new Date()).then(function (response) {
                ...
                _this.logError('HTML file for <strong>' + component.name + '</strong> component failed to load from <code>' + component_path + '/html.html</code>');
            });

            // Get and set component description
            _this.$http.get(component_path + '/description.html' + '?cb=' + new Date()).then(function (response) {
                component.description = marked(response.data);
                _this.areComponentsLoaded();
            }, function () {
                _this.logError('Description file for <strong>' + component.name + '</strong> component failed to load from <code>' + component_path + '/description.html</code>');
            });
        },
